...
def handle_connect():
    ...
    global ARIS_instance, ARIS_loop, ARIS_thread
    client_sid = request.sid
    print(f"\n--- handle_connect called for SID: {client_sid} ---")

    if ARIS_thread is None or not ARIS_thread.is_alive():
        print(f"    Asyncio thread not running. Starting new loop and thread.")
        ARIS_loop = asyncio.new_event_loop()
        ARIS_thread = threading.Thread(target=run_asyncio_loop, args=(ARIS_loop,), daemon=True)
        ARIS_thread.start()
        print("    Started asyncio thread.")
        socketio.sleep(0.1)

    if ARIS_instance is None:
        print(f"    Creating NEW ARIS instance for SID: {client_sid}")
        if not ARIS_loop or not ARIS_loop.is_running():
             print(f"    ERROR: Cannot create ARIS instance, asyncio loop not ready for SID {client_sid}.")
             emit('error', {'message': 'Assistant initialization error (loop).'}, room=client_sid)
             return

        try:
            ARIS_instance = ARIS(socketio_instance=socketio, client_sid=client_sid)
            future = asyncio.run_coroutine_threadsafe(ARIS_instance.start_all_tasks(), ARIS_loop)
            print("    ARIS instance created and tasks scheduled.")
        except ValueError as e:
            print(f"    ERROR initializing ARIS (ValueError) for SID {client_sid}: {e}")
            emit('error', {'message': f'Failed to initialize assistant: {e}'}, room=client_sid)
            ARIS_instance = None
            return
        except Exception as e:
            print(f"    ERROR initializing ARIS (Unexpected) for SID {client_sid}: {e}")
            emit('error', {'message': f'Unexpected error initializing assistant: {e}'}, room=client_sid)
            ARIS_instance = None
            return
    else:
        print(f"    ARIS instance already exists. Updating SID from {ARIS_instance.client_sid} to {client_sid}")
        ARIS_instance.client_sid = client_sid

    if ARIS_instance:
        emit('status', {'message': 'Connected to ARIS Assistant'}, room=client_sid)
...
def handle_disconnect():
    ...
    if ARIS_instance and ARIS_instance.client_sid == client_sid:
        print(f"    Designated client {client_sid} disconnected. Attempting to stop ARIS.")
        if ARIS_loop and ARIS_loop.is_running():
            future = asyncio.run_coroutine_threadsafe(ARIS_instance.stop_all_tasks(), ARIS_loop)
            try:
                future.result(timeout=10)
                print("    ARIS tasks stopped successfully.")
            except TimeoutError:
                print("    Timeout waiting for ARIS tasks to stop.")
            except Exception as e:
                print(f"    Exception during ARIS task stop: {e}")
            finally:
                 pass # Keep loop running

        else:
             print(f"    Cannot stop ARIS tasks: asyncio loop not available or not running.")

        ARIS_instance = None
        print("    ARIS instance cleared.")

    elif ARIS_instance:
         print(f"    Disconnecting client {client_sid} is NOT the designated client ({ARIS_instance.client_sid}). ARIS remains active.")
    else:
         print(f"    Client {client_sid} disconnected, but no active ARIS instance found.")
...
def handle_text_message(data):
    ...
    if ARIS_instance and ARIS_instance.client_sid == client_sid:
        ...
        print(f"    ARIS instance not ready or SID mismatch for text message from {client_sid}.")
...
def handle_transcribed_text(data):
    ...
    if transcript and ARIS_instance and ARIS_instance.client_sid == client_sid:
         if ARIS_loop and ARIS_loop.is_running():
            # Process transcript with end_of_turn=True implicitly handled in process_input -> run_gemini_session
            asyncio.run_coroutine_threadsafe(ARIS_instance.process_input(transcript, is_final_turn_input=True), ARIS_loop)
            print(f"    Transcript forwarded to ARIS for SID: {client_sid}")
         else:
...
def handle_video_frame(data):
    ...
    if frame_data_url and ARIS_instance and ARIS_instance.client_sid == client_sid:
        if ARIS_loop and ARIS_loop.is_running():
            print(f"Received video frame from {client_sid}, forwarding...") # Optional: very verbose
            asyncio.run_coroutine_threadsafe(ARIS_instance.process_video_frame(frame_data_url), ARIS_loop)
        pass
...
def handle_video_feed_stopped():
    ...
    if ARIS_instance and ARIS_instance.client_sid == client_sid:
        if ARIS_loop and ARIS_loop.is_running():
            # Call a method on ARIS instance to clear its video queue
            asyncio.run_coroutine_threadsafe(ARIS_instance.clear_video_queue(), ARIS_loop)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if ARIS_instance:
             print("Attempting to stop active ARIS instance on server shutdown...")
             if ARIS_loop and ARIS_loop.is_running():
                 future = asyncio.run_coroutine_threadsafe(ARIS_instance.stop_all_tasks(), ARIS_loop)
                 try:
                     ...
                     print("Timeout stopping ARIS tasks during shutdown.")
                 except Exception as e:
                     print(f"Exception stopping ARIS tasks during shutdown: {e}")
             else:
                 print("Cannot stop ARIS instance: asyncio loop not available.")
             ARIS_instance = None

        if ARIS_loop and ARIS_loop.is_running():
             print("Stopping asyncio loop from main thread...")
             ARIS_loop.call_soon_threadsafe(ARIS_loop.stop)
             if ARIS_thread and ARIS_thread.is_alive():
                 ARIS_thread.join(timeout=5)
                 if ARIS_thread.is_alive():
